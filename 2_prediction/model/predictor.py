...
def transformation():
    ...
    # Accept an S3 location containing a CSV file 
    ...
    target_stock = savedParams.get('target_stock')
    covariate_stocks = savedParams.get('covariate_stocks')
    covariates = covariate_stocks.split(',')
    for i, cov in enumerate(covariates):
        covariates[i] = cov.strip()        
    target_column = savedParams.get('target_column')
    covariate_columns = savedParams.get('covariate_columns', 'open, low, high')
    ...
    print('Reading data from CSV file at', file)
    traindata = pd.read_csv(file, index_col = 0, parse_dates=True)
    traindata['ticker'] = traindata['ticker'].apply(lambda x: str(x))
    traindata['ticker'] = traindata['ticker'].apply(lambda x: '0'*(6-len(x)) + x)
    ...
    test_main.drop(['ticker', target_column], 1, inplace=True)    
    ...
    train_main = traindata[traindata['ticker'] == target_stock].copy()    
    ...
    for n, covariate_stock in enumerate(covariates):
        if target_stock != covariate_stock:
            ...
            if len(logoutput) == 7:
                print(logoutput, '')
                logoutput = []
            exo = data[data['ticker'] == covariate_stock].copy()
            ...
            train_exo = traindata[traindata['ticker'] == covariate_stock].copy()
            
            train_exo['TargetMetric'] = train_exo[target_column]
            train_exo.drop(['ticker', target_column], 1, inplace=True)
            ...
            for col in exo.columns.values:
                ...
                test_exo = test_exo.combine_first(metric_col)  
                
    print("\t".join(logoutput))
    test_exo = test_exo.loc[test_main.index]
    test_exo.fillna(method='ffill', inplace=True)
